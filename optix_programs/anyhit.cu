...
extern "C" __global__ void __anyhit__shadow_alpha() {
    ...
    // Alpha-mask cutout test FIRST: where alpha < cutoff NO surface exists,
    // so the intersection is ignored before any occlusion logic runs. The
    // alpha fetch mirrors __anyhit__alpha exactly — the two anyhits must
    // agree on where surface exists, or NEE sees a blocker where path
    // transport sees a hole (permanent darkening for MASK + transmissive
    // materials, whose MIS halves then integrate different emitters).
    if (material.alphaMode == ALPHA_MODE_MASK) {
        // Get instance ID for buffer lookup
        const unsigned int instanceId = optixGetInstanceId();
        ...
        // Discard if below cutoff (no surface — shadow ray passes through)
        if (alpha < material.alphaCutoff) {
            optixIgnoreIntersection();
        }
    }
    ...
    // Opaque or surviving MASK texel: accept hit — shadow is blocked.
}
